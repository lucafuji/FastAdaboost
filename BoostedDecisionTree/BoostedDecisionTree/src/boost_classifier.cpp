...
std::pair<float,long_vector_ptr_type>  boost_classifier::learn(processed_data_ptr p_data,vector_ptr_type weights)
{
    ...
    for(int t = 0;t<rounds;t++){
        ...
        uvector tmp_vec(-(*bc_weights)(t)*ublas::element_prod(*(p_data->tlabel),*h)); // -a(t)*y(i)*h(xi)
        std::transform(tmp_vec.begin(),tmp_vec.end(),exp_vec.begin(),::exp);//exp(-a(t)*y(i)*h(xi))
        *d=ublas::element_prod(*d,exp_vec)/z;
#ifdef DEBUG
        std::cout<<t<<" t"<<std::endl;
        std::cout<<error<<" error"<<std::endl;
        std::cout<<(*bc_weights)(t)<<" a"<<std::endl;
        std::cout<<*h<<" h"<<std::endl;
        std::cout<<*H<<" H"<<std::endl;
        std::cout<<z<<" z "<<std::endl;
        std::cout<<*d<<" weights"<<std::endl;
#endif
        std::cout<<t+1<<"/"<<rounds<<" "<<(t+1)/1.0/rounds*100<<"% completed"<<std::endl;
    }
...
#endif
    return std::make_pair(ublas::sum(*(matrix_util::compare_vector<longvector,longvector,longvector,long_not_equal>(p_data->tlabel->begin(),p_data->tlabel->end(), h->begin())))/1.0/nrow, h);
}
