...
processed_data_ptr stump_classifier::preprocess(matrix_ptr_type data,long_vector_ptr_type labels,float eps)
{
#ifdef DEBUG
...
std::pair<float,long_vector_ptr_type> stump_classifier::learn(processed_data_ptr p_data,vector_ptr_type weights)
{
#ifdef DEBUG
    ...
    for (long col = 0;col < ncol; ++col){
        ...
        for(long j = 0; j<(*stump)["l"]->size(); ++j){
            for(long k = (*(*stump)["lstarts"])[j]; k<(*(*stump)["lends"])[j]+1; ++k){
                d[j] = d[j] + dist[k];
            }
        }
