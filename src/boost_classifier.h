...
class boost_classifier : public classifier {
 public:
  ...
  boost_classifier(long rounds = 20)
      : rounds(rounds), bc_weights(vector_ptr_type(new uvector(rounds))) {
  };
  ...
  processed_data_ptr preprocess(matrix_ptr_type data,
                                ...
                                float eps);
  std::pair<float, long_vector_ptr_type> learn(processed_data_ptr p_data,
