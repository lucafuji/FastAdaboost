...
tuple_ptr_type data_processor::z_score(matrix_ptr_type data) {
  ...
  for (unsigned j = 0; j < ncol; ++j) {
    ...
    for (unsigned i = 0; i < nrow; ++i) {
      ((*mean_vector)(j)) = (1 - 1.0 / (i + 1)) * ((*mean_vector)(j))
          + 1.0 / (i + 1) * ((*data)(i, j));
    }
  }
  ...
  for (unsigned j = 0; j < ncol; ++j) {
    (*std_vector)(j) = 0;
    for (unsigned i = 0; i < nrow; ++i) {
      (*std_vector)(j) = (1 - 1.0 / (i + 1)) * ((*std_vector)(j))
          + 1.0 / (i + 1) * pow((*data)(i, j) - ((*mean_vector)(j)), 2.0);
    }
  }
  ...
  for (unsigned i = 0; i < nrow; ++i) {
    for (unsigned j = 0; j < ncol; ++j) {
      (*new_matrix)(i, j) =
          ((*data)(i, j) - ((*mean_vector)(j))) / ((*std_vector)(j));
    }
  }

  bt::tuple<matrix_ptr_type, vector_ptr_type, vector_ptr_type> *ptuple =
      new bt::tuple<matrix_ptr_type, vector_ptr_type, vector_ptr_type>(
          new_matrix,
          mean_vector,
          std_vector);
...
std::pair<matrix_ptr_type,
          long_vector_ptr_type> data_processor::load_data(std::istream &input_stream,
  ...
  if (sparse) {
    data = matrix_ptr_type(new umatrix(ublas::zero_matrix<float>(nrow,
                                                                 number_features)));
    ...
    for (unsigned i = 0; i < nrow; ++i) {
      ...
      for (value_iter = input_vectors[i].begin(), index_iter =
                                                      input_vectors_index[i].begin();
           value_iter != input_vectors[i].end(); ++value_iter, ++index_iter) {
    ...
    for (unsigned i = 0; i < nrow; ++i) {
      std::copy(input_vectors[i].begin(),
                input_vectors[i].end(),
                ublas::row(*data, i).begin());
    }
  }
  //copy matrix
...
matrix_ptr_type data_processor::re_zscore(matrix_ptr_type data,
