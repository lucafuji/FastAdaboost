...
int main(int argc, const char *argv[]) {
  ...
  desc.add_options()
      ...
      ("final_predictor,f",
       po::value<std::string>(),
       "path to store the predictor")
      ("output,o",
       po::value<std::string>(),
       "path to store the predicted labels")
      ("do_feature_hashing,d", "whether to do feature hashing")
      ("num_hash_bits,n",
       po::value<long>()->default_value(0),
       "number of hash bits")
      ("sparse,s", "whether the input data is sparse")
      ("nrounds,r",
       po::value<long>()->default_value(2),
       "number of rounds for adaboost")
      ("normalize,z", "normalize the data by z_score")
      ("episilon,e",
       po::value<float>()->default_value(1e-6),
       "the difference between values that we accept")
  ...
  if (vm.count("help")) {
    ...
    cout
        << "For predicting: cat inputfile| ./boost_tree -p predictor -o outputfile"
        << endl;
    return 0;
  }
