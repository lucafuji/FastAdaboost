...
void feature_hashing_classifier::compute_hash_matrix(long_vector_ptr_type feature_index_vector) {
  ...
  cmatrix_ptr_type
      hash_matrix(new cmatrix(num_hash_bits, n_feature, n_feature));
  for (longvector::iterator iter = feature_index_vector->begin();
       iter != feature_index_vector->end(); ++iter) {
...
processed_data_ptr feature_hashing_classifier::preprocess(matrix_ptr_type data,
  ...
  matrix_ptr_type hashed_data(new umatrix(ublas::trans(ublas::prod(*hash_matrix,
                                                                   ublas::trans(
                                                                       *data)))));
...
std::pair<float, long_vector_ptr_type> feature_hashing_classifier::learn(
...
long feature_hashing_classifier::predict(vector_ptr_type instance) {
  vector_ptr_type hashed_data(new uvector(ublas::trans(ublas::prod(*hash_matrix,
                                                                   ublas::trans(
                                                                       *instance)))));
