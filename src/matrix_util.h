...
class matrix_util {
 public:
  ...
  static std::pair<long_matrix_ptr_type,
                   boost::shared_ptr<Matrix_Type> > arg_sort(boost::shared_ptr<
      Matrix_Type> matrix, int axis = 0);

  template<class InputType, class ReturnType>
  static boost::shared_ptr<ReturnType> non_zeros(typename InputType::iterator begin,
                                                 typename InputType::iterator end,
                                                 int extra_dimension = 0,
                                                 double eps = EPS);

  template<class InputType, class ReturnType, class Predicate>
  static boost::shared_ptr<ReturnType> judge_vector(typename InputType::iterator begin,
                                                    typename InputType::iterator end,
                                                    int negative_value = 0,
                                                    Predicate predicate = Predicate());

  template<class InputType, class ReturnType, class Predicate>
  static std::pair<boost::shared_ptr<ReturnType>,
                   boost::shared_ptr<ReturnType> > judge_vector_pair(typename InputType::iterator begin,
                                                                     typename InputType::iterator end,
                                                                     int negative_value = 0,
                                                                     Predicate predicate = Predicate());

  template<class Type1, class Type2, class ReturnType, class Predicate>
  static boost::shared_ptr<ReturnType> compare_vector(typename Type1::iterator begin1,
                                                      typename Type1::iterator end1,
                                                      typename Type2::iterator begin2,
                                                      Predicate predicate = Predicate());

};

template<class Matrix_Type>
std::pair<long_matrix_ptr_type,
          boost::shared_ptr<Matrix_Type> > matrix_util::arg_sort(boost::shared_ptr<
    Matrix_Type> matrix, int axis) {
  ...
  if (axis == 0) {
    for (long j = 0; j < ncol; j++) {
      ...
      sort(boost::iterator_range<ublas::unbounded_array<std::pair<float,
                                                                  long> >::iterator>(
          valueIndexVectors.begin(),
          valueIndexVectors.end()));
      for (long i = 0; i < nrow; i++) {
...
template<class InputType, class ReturnType>
boost::shared_ptr<ReturnType> matrix_util::non_zeros(typename InputType::iterator begin,
                                                     typename InputType::iterator end,
                                                     int extra_dimension,
                                                     double eps) {
  ...
  boost::shared_ptr<ReturnType>
      result(new ReturnType(result_vector.size() + extra_dimension));
  std::copy(result_vector.begin(),
            result_vector.end(),
            result->begin() + extra_dimension);
  return result;
}

template<class InputType, class ReturnType, class Predicate>
boost::shared_ptr<ReturnType> matrix_util::judge_vector(typename InputType::iterator begin,
                                                        typename InputType::iterator end,
                                                        int negative_value,
                                                        Predicate predicate) {
  boost::shared_ptr<ReturnType>
      result(new ReturnType(std::distance(begin, end)));
  typedef typename InputType::iterator Iterator;
...
template<class InputType, class ReturnType, class Predicate>
std::pair<boost::shared_ptr<ReturnType>,
          boost::shared_ptr<ReturnType> > matrix_util::judge_vector_pair(
    typename InputType::iterator begin,
    typename InputType::iterator end,
    int negative_value,
    Predicate predicate) {
  typedef typename InputType::iterator Iterator;
  typedef typename ReturnType::iterator ReturnIterator;
  boost::shared_ptr<ReturnType>
      result1(new ReturnType(std::distance(begin, end)));
  boost::shared_ptr<ReturnType>
      result2(new ReturnType(std::distance(begin, end)));
  ReturnIterator result1_iter = result1->begin();
  ReturnIterator result2_iter = result2->begin();
  for (Iterator iter = begin; iter != end;
       ++iter, ++result1_iter, ++result2_iter) {
    if (predicate(*iter)) {
      *result1_iter = 1;
...
template<class Type1, class Type2, class ReturnType, class Predicate>
boost::shared_ptr<ReturnType> matrix_util::compare_vector(typename Type1::iterator begin1,
                                                          typename Type1::iterator end1,
                                                          typename Type2::iterator begin2,
                                                          Predicate predicate) {
  ...
  typedef typename ReturnType::iterator ReturnIterator;
  boost::shared_ptr<ReturnType>
      result(new ReturnType(std::distance(begin1, end1)));

  Iterator2 iter2 = begin2;
  ReturnIterator result_iter = result->begin();
  for (Iterator1 iter1 = begin1; iter1 != end1;
       ++iter1, ++iter2, ++result_iter) {
