...
class feature_hashing_classifier : public classifier {
 public:
  ...
  feature_hashing_classifier(classifier *basic_classifier = NULL,
                             long num_hash_bits = 100) : basic_classifier(
      basic_classifier), num_hash_bits(num_hash_bits) {
    seed = time(NULL);
    len = 64;
  }
  ...
  virtual processed_data_ptr preprocess(matrix_ptr_type data,
                                        long_vector_ptr_type labels,
                                        float eps);
  virtual std::pair<float,
                    long_vector_ptr_type> learn(processed_data_ptr p_data,
 ...
 private:
  ...
  long hash_value(long *feature_index_ptr) {
    return MurmurHash64A(feature_index_ptr,
                         len,
                         seed);
  }
